...
- **Мемуарник** (`/memoir`) — каждый вечер сначала хронологический список занятий по ответам трекера, затем вопрос «что было самым ярким?», анализ ценностей. Ответ записывается только через Telegram Reply на сам вопрос, поэтому незакрытый prompt не перехватывает обычные задачи и напоминания. Недельный обзор по воскресеньям отсортирован по дате (сверху — раньше)
